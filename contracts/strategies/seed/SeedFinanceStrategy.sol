...
contract SeedFinanceStrategy is IStrategyV2, RewardTokenProfitNotifier, Ownable {

    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    event Liquidating(address, uint256);
    IERC20 valutUnderlying;
    ...
    function investAllUnderlying() public restricted {
        ...
        for (uint256 i = 0; i < market.length; i++) {
            ...
            valutUnderlying.safeApprove(address(market[i].investRouter), 0);
            valutUnderlying.safeApprove(
            ...
            if (market[i].marketType == MarketType.Swap) {
                ...
                require(
                    CErc20Interface(market[i].investRouter).underlying() ==
                        address(valutUnderlying),
                    "token not found"
                );
  ...
  function withdrawToVault(uint256 amountUnderlying, uint256) public restricted {
    ...
    if (amountUnderlying > 0) {
        valutUnderlying.safeTransfer(vault, amountUnderlying);
    }
