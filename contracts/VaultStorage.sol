...
contract VaultStorage is Initializable {
  ...
  function _setSeedPoolAddress(address _address) internal {
    setAddress(_SEED_POOL_ADDRESS_SLOT, _address);
  }

  function _seedPoolAddress() internal view returns (address) {
    return getAddress(_SEED_POOL_ADDRESS_SLOT);
  }

  function _setSeedPoolId(uint256 _poolId) internal {
    setUint256(_SEED_POOL_ID_SLOT, _poolId);
  }
